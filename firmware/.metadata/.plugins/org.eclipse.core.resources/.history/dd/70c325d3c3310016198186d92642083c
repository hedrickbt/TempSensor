...
static const uint8_t SystemMessageString[] = "-----------------------------------\r\n"
											...
											COMPILED_DATA_TIME "\r\n"
											"-----------------------------------\r\n"
											"S1 LED toggle : U0 = led state \r\n"
											"S2 ADC On \r\n"
											"S3 ADC Off \r\n"
											"S4 ADC Sample : U0 = channel.  Channel 16 internal temp sensor.\r\n";
...
static int_fast8_t RunCommand(ListOfParameterStructureType *source)
{
	...
	switch ( source->List[0].Value.i32_t[0] )
	{
		case Command_LedControl: // control the LED
			...
			break;
		case Command_AdcOn: // Turn the ADC on
			ADC_On();
			break;
		case Command_AdcOff: // Turn the ADC off
			ADC_Off();
			break;
